...
import { Download, FileDown, Loader2, TrendingUp, TrendingDown, Minus } from "lucide-react";
import {
  BarChart, Bar,
  LineChart, Line,
  AreaChart, Area,
...
function computeStats(data: DataPoint[]): Stats | null {
...
function KeyStatsBar({ stats, unit }: { stats: Stats; unit: string }) {
  const direction =
    Math.abs(stats.changePct) < 2 ? "stable" : stats.changePct > 0 ? "up" : "down";

  const TrendIcon =
    direction === "up" ? TrendingUp : direction === "down" ? TrendingDown : Minus;
  const trendColor =
    direction === "up" ? "text-blue-600" : direction === "down" ? "text-emerald-600" : "text-gray-500";
  const trendLabel =
    direction === "up" ? "Increasing" : direction === "down" ? "Decreasing" : "Stable";
  const yearSpan = Number(stats.latestYear) - Number(stats.firstYear);

  return (
    <div className="mb-5 space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: `Latest (${stats.latestYear})`, value: formatValue(stats.latestValue) },
          { label: `First (${stats.firstYear})`, value: formatValue(stats.firstValue) },
          { label: "Total Change", value: `${stats.changePct > 0 ? "+" : ""}${stats.changePct.toFixed(1)}%`, sub: `${stats.firstYear} → ${stats.latestYear}` },
          { label: "Observations", value: String(stats.nObs) },
        ].map((c) => (
          <div key={c.label} className="bg-gray-50 rounded-xl px-4 py-3">
            ...
            {"sub" in c && c.sub && <p className="text-[10px] text-gray-400 mt-0.5">{c.sub}</p>}
          </div>
        ))}
      </div>
      <div className="bg-gray-50 rounded-xl px-4 py-3 flex items-center gap-3">
        <TrendIcon className={`w-5 h-5 flex-shrink-0 ${trendColor}`} strokeWidth={2} />
        <div>
        ...
        {yearSpan > 0 && (
          <p className="text-xs text-gray-400 ml-2">
            {Math.abs(stats.changePct).toFixed(1)}% {direction === "up" ? "rise" : direction === "down" ? "decline" : "variation"} over {yearSpan} year{yearSpan !== 1 ? "s" : ""}
          </p>
        )}
      </div>
    </div>
...
function VerdictBadge({ verdict }: { verdict: string }) {
  ...
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${styles}`}>{verdict}</span>;
}

// ─── Chart renderer ───────────────────────────────────────────────────────────

function ChartRenderer({
  type, data, unit, citations, sourceLabel,
}: {
  type: ChartType;
  data: DataPoint[];
  unit: string;
  ...
  const commonAxis = (
    <>
      ...
      <YAxis tick={{ fontSize: 12, fill: "#6b7280" }} axisLine={false} tickLine={false} width={56} tickFormatter={yTickFormatter} />
      <Tooltip
      ...
      />
      {citations && (
        <Legend wrapperStyle={{ fontSize: 11, color: "#9ca3af" }} formatter={() => `Source: ${sourceLabel}`} />
      )}
    </>
  );
...
function LoadedState({ title, dataset, country, verdict, onDataLoaded }: LoadedProps) {
  ...
  const downloadChart = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        a.href = pngUrl; a.download = `${title.replace(/[^a-z0-9]/gi, "_")}.png`; a.click();
        URL.revokeObjectURL(pngUrl);
      }, "image/png");
    };
    img.onerror = () => {
      const a = document.createElement("a");
      a.href = svgUrl; a.download = `${title.replace(/[^a-z0-9]/gi, "_")}.svg`; a.click();
      URL.revokeObjectURL(svgUrl);
    };
    img.src = svgUrl;
  };
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
      ...
      <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={citations} onChange={(e) => setCitations(e.target.checked)}
              className="w-4 h-4 rounded accent-blue-600 cursor-pointer" />
            <span className="text-sm text-gray-700 font-medium">Citations</span>
          </label>
          ...
          <button onClick={downloadChart} disabled={!hasData} title="Download PNG"
            ...
            <Download className="w-4 h-4" strokeWidth={2} /> PNG
          </button>
          <button onClick={downloadCSV} disabled={!hasData} title="Download CSV"
            className="flex items-center gap-1.5 text-sm text-gray-600 border border-gray-200 rounded-lg px-3 py-1.5 hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            <FileDown className="w-4 h-4" strokeWidth={2} /> CSV
          </button>
        </div>
      </div>

      {/* Key statistics */}
      {hasData && stats && <KeyStatsBar stats={stats} unit={unit} />}
      ...
      {loading ? (
        ...
        <div className="h-48 flex flex-col items-center justify-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">{dataset} · {country}</p>
          </div>
        </div>
      ) : (
        <div ref={chartRef} className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <ChartRenderer type={chartType} data={data} unit={unit} citations={citations} sourceLabel={sourceOrg || dataset} />
          </ResponsiveContainer>
        </div>
      )}
      ...
      {hasData && (
        <div className="mt-4 border-t border-gray-100 pt-4">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Data Provenance</p>
          <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            <div>
              <dt className="text-gray-400">Source organisation</dt>
              <dd className="text-gray-700 font-medium mt-0.5">
                {licenseUrl
                  ? <a href={licenseUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{sourceOrg || dataset}</a>
                  : (sourceOrg || dataset)}
              </dd>
            </div>
            <div>
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
      <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
        ...
        <div className="w-full max-w-md space-y-3">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider text-left mb-1">Dataset ID</label>
            <input type="text" value={datasetInput} onChange={(e) => setDatasetInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleLoad()} placeholder="e.g., sdg/SH_DYN_MORT"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider text-left mb-1">Country (ISO3 code)</label>
            <input type="text" value={countryInput} onChange={(e) => setCountryInput(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleLoad()} placeholder="e.g., KEN" maxLength={3}
              className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm" />
          </div>
          <button onClick={handleLoad} disabled={!canSubmit}
...
export default function VisualizationCard({ dataset, country, name, verdict, onManualLoad, onDataLoaded }: Props) {
  if (!dataset || !country) {
    return <EmptyState onLoad={onManualLoad ?? (() => {})} />;
  }
  const displayName = name ? `${name} (${country})` : dataset;
  return (
    <LoadedState title={displayName} dataset={dataset} country={country} verdict={verdict} onDataLoaded={onDataLoaded} />
  );
}
