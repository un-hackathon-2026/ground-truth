...
WB_TO_COMMONS: dict[str, str] = {
    # --- VERIFIED via live probe (variables Kenya actually returns data for) ---
    # NOTE: this Commons instance serves SDG ids WITHOUT the "undata/" prefix,
    # and the under-5 rate has NO __SEX suffix. Confirmed by querying the
    # live /v2/observation "what variables exist for KEN" list.
    "SP.POP.TOTL": "Count_Person",
    #   ^ total population — confirmed live (5 sources, Kenya)
    "SH.DYN.MORT": "sdg/SH_DYN_MORT.AGE--Y0T4",
    #   ^ under-5 mortality rate, age 0-4 (no sex suffix = the aggregate)
    "SH.STA.MMRT": "sdg/SH_STA_MORT.SEX--F",
    #   ^ maternal mortality ratio (maternal = inherently female = the total)

    # --- NO CLEAN COMMONS EQUIVALENT (intentionally omitted, not fabricated) ---
    # "SP.DYN.LE00.IN": life expectancy — Commons has many age/sex-sliced
    #     LIFE_* variants but no single clean total matching the WB indicator.
    # "SH.XPD.CHEX.GD.ZS": health expenditure — no clean aggregate match.
    # These return NO_COMMONS_EQUIVALENT (honest "we don't have a match").
}
